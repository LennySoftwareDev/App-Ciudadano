...
import '../../downloadpdf/ArgumentWebView.dart';
import '../../downloadpdf/downloadpdf_screen.dart';

class TransitScreen extends StatefulWidget {
  final GlobalKey _mainInfoFormKey = GlobalKey<FormState>();

  TransitScreen({Key? key}) : super(key: key);
...
class _TransitScreenState extends State<TransitScreen> {
  ...
  final TransitBloc _transitBloc = injector.resolve();
  bool onInit = true;
  ...
  Widget _body(TransitState state) {
        ...
        child: Column(
          children: [
            ...
            SearchCitations(
              ...
              title: ('Comparendos de transito'),
              parentTitle: '',
              searchQuery: (value) {
                if (value != "") {
                  this._transitBloc.searchTransit(value);
                } else {
                  this._transitBloc.getCitations();
                }
              },
              results: Expanded(
                child: Padding(
  ...
  Widget _cardCitation(TransitState state) {
      ...
      itemBuilder: (context, index) {
        return Padding(
          ...
          child: Card(
            elevation: 8,
            child: ListTile(
              onTap: (() => utils.visualizePDF(
                  state.getCitations[index].pdfUrl, context)),
              trailing: Icon(
  ...
  void _removeFilter(BuildContext contexts, TransitState state) {
    _transitBloc.initBloc();
  }
                          ...
                          children: [
                            ...
                            ElevatedButton(
                                ...
                                onPressed: () {
                                  ...
                                  _transitBloc.filterSearchCourses(
  ...
  dialog(
      BuildContext context, String mensaje, TransitState state, Color color) {
    showDialog(
      context: context,
